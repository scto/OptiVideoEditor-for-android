...
public final class OptiBackgroundExecutor {
   ...
   *     <p>if <code>delay</code> is strictly positive and the current executor does not support
   *     scheduling (if Executor has been called with such an executor)
   * @return Future associated to the running task
   * @throws IllegalArgumentException if the current executor set by Executor does not support
   *     scheduling
   */
  ...
  /**
   * Execute a task after (at least) its delay <strong>and</strong> after all tasks added with the
   * same non-null <code>serial</code> (if any) have completed execution.
   *
   * @param task the task to execute
   * @throws IllegalArgumentException if <code>task.delay</code> is strictly positive and the
   *     current executor does not support scheduling (if Executor has been called with such an
   *     executor)
   */
  ...
  /**
   * Indicates whether a task with the specified <code>serial</code> has been submitted to the
   * executor.
   *
   * @param serial the serial queue
   * @return <code>true</code> if such a task has been submitted, <code>false</code> otherwise
   */
  ...
  /**
   * Retrieve and remove the first task having the specified <code>serial</code> (if any).
   *
   * @param serial the serial queue
