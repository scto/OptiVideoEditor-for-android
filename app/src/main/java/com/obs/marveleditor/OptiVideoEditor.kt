...
class OptiVideoEditor private constructor(private val context: Context) {
    ...
    fun addBorder(isBorder: Boolean): OptiVideoEditor {
        if (isBorder) this.border = BORDER_FILLED else this.border = BORDER_EMPTY
        return this
    }
    ...
    fun main() {
        ...
        when (type) {
            ...
            OptiConstant.VIDEO_TEXT_OVERLAY -> {
                // Text overlay on video - Need video file, font file, text, text color, text size,
                // border if needed, position to apply & output file
                cmd =
                    arrayOf(
                        "-y",
                        "-i",
            ...
            OptiConstant.VIDEO_CLIP_ART_OVERLAY -> {
                // Clipart overlay on video - Need video file, image path, position to apply &
                // output file
                cmd =
                    arrayOf(
                        "-y",
            ...
            OptiConstant.VIDEO_PLAYBACK_SPEED -> {
                // Video playback speed - Need video file, speed & tempo value according to playback
                // and output file
                cmd =
                    if (havingAudio) {
                        arrayOf(
